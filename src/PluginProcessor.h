...
class RGainAudioProcessor final : public juce::AudioProcessor
{
public:
    RGainAudioProcessor();
    ~RGainAudioProcessor() override = default;
...
private:
    ...
    rgain::dsp::GainProcessor gainProcessor;

    std::atomic<float> leftMeterLevel { 0.0f };
    std::atomic<float> rightMeterLevel { 0.0f };

    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(RGainAudioProcessor)
};
